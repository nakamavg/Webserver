...
/*   Updated: 2024/09/17 16:00:57 by anurtiag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_sockets()
{
    ...
    std::string html = ft_read("ejemplo.html");
    std::string html1 = ft_read("holamundo.html");
    std::string response =
    "HTTP/1.1 200 OK\r\n"
    "Content-Type: text/html\r\n"
    "Content-Length: " + html + "\r\n"
        ...
        for(int i = 0; i < num_events; i++)
        {
            ...
            if(events[i].events & EPOLLIN)//hay un evento de escritura
            {
                ...
                if(b.find("/ ") != std::string::npos)
                    ...
                    send(clientfd[i], response1.c_str(), response1.length(), 0);
                else
